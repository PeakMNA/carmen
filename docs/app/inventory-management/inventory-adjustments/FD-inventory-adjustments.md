...
```mermaid
flowchart TD
    Start([User accesses<br/>Adjustment List]) --> LoadData["Load all adjustments:<br/><br/>mockAdjustments = [<br/>  {id, date, type, status,<br/>   location, reason, items, value},<br/>  ... 8 adjustments<br/>]"]
    ...
    InitState --> DisplayTable["Display table:<br/>- 8 adjustments visible<br/>- Default sort: Date desc<br/>- Search bar ready<br/>- Filter popover closed"]
...
```mermaid
flowchart TD
    Start([User clicks<br/>adjustment row]) --> CaptureID["Capture adjustment ID:<br/>const adjId = adjustment.id<br/>(e.g., 'ADJ-2024-001')"]
...
- **Breadcrumb**: Home → Inventory → Adjustments → ADJ-2024-001
...
flowchart TD
    ...
    PostValid -->|Yes| ConfirmPost["Show confirmation dialog:<br/>'Post adjustment ADJ-2024-001?<br/><br/>This will:<br/>- Generate journal entries<br/>- Update stock balances<br/>- Lock the adjustment<br/><br/>Continue?'"]
    ...
    ClickDelete --> ConfirmDelete["Show confirmation dialog:<br/>'Delete adjustment ADJ-2024-001?<br/><br/>This cannot be undone.<br/><br/>Continue?'"]
    ...
    ClickVoid --> ConfirmVoid["Show confirmation dialog:<br/>'Void adjustment ADJ-2024-001?<br/><br/>Enter reason for void:<br/>[Text input field]<br/><br/>This will:<br/>- Reverse journal entries<br/>- Reverse stock changes<br/>- Mark as voided<br/><br/>Continue?'"]
...
This diagram shows the workflow for creating a new inventory adjustment (future feature).

```mermaid
flowchart TD
    Start([User clicks<br/>New Adjustment]) --> InitForm["Initialize form:<br/><br/>Default values:<br/>- adjustment_date: today<br/>- adjustment_type: null<br/>- adjustment_status: 'DRAFT'<br/>- location_id: user's default<br/>- department_id: user's dept<br/>- adjustment_reason: null<br/>- items: []"]

    InitForm --> RenderForm["Render form sections:<br/><br/>Section 1: Header<br/>- Date picker<br/>- Type selector (IN/OUT)<br/>- Location dropdown<br/>- Department dropdown<br/><br/>Section 2: Reason<br/>- Reason dropdown<br/>- Description text area<br/><br/>Section 3: Items<br/>- Item selection<br/>- Add Item button"]

    RenderForm --> UserInput["User fills form fields:<br/><br/>React Hook Form:<br/>- Field validation<br/>- Error messages<br/>- Disable submit until valid"]

    UserInput --> AddItems{User adds<br/>items?}

    AddItems -->|Yes| ItemDialog["Show Add Item dialog:<br/><br/>Fields:<br/>- Item selector (search)<br/>- Location (within facility)<br/>- Lot number<br/>- Quantity<br/>- Unit cost<br/>- Expiry date (optional)<br/><br/>Validation:<br/>- Required fields<br/>- Quantity > 0<br/>- Unit cost >= 0"]
    ...
    ValidateItem -->|Yes| AddToList["Add to items array:<br/><br/>items.push({<br/>  item_id,<br/>  product_name,<br/>  sku,<br/>  location_type,<br/>  location_code,<br/>  lot_no,<br/>  quantity,<br/>  uom,<br/>  unit_cost,<br/>  total_cost: qty × cost<br/>})"]
    ...
    UpdateTotals --> DisplayItems["Display items table:<br/>- Product name<br/>- Quantity<br/>- Unit cost<br/>- Total cost<br/>- Remove button per row<br/><br/>Footer: Grand totals"]
    ...
    SaveDraft -->|Yes| ValidateForm["Validate entire form:<br/><br/>1. Required fields filled<br/>2. At least one item<br/>3. All item data valid<br/>4. Totals calculated<br/>5. Type matches totals"]
    ...
    FormValid -->|Yes| GenerateID["Generate adjustment ID:<br/><br/>adjId = 'ADJ-' + year + '-' +<br/>  paddedSequence<br/><br/>Example: 'ADJ-2024-009'"]

    GenerateID --> SaveAdjustment["Save adjustment:<br/><br/>INSERT INTO tb_inventory_adjustment<br/>VALUES (adjustment data)<br/><br/>INSERT INTO tb_adjustment_item<br/>VALUES (items data)<br/><br/>INSERT INTO<br/>  tb_adjustment_stock_movement<br/>VALUES (movement data)"]

    SaveAdjustment --> CreateSuccess["Show success message:<br/>'Adjustment ADJ-2024-009<br/>saved as Draft'<br/><br/>Action buttons:<br/>- View adjustment<br/>- Create another"]
...
flowchart TD
    ...
    CreateHeader --> GenJournalNo["Generate journal number:<br/><br/>year = EXTRACT(YEAR FROM date)<br/><br/>sequence = MAX(journal_no) + 1<br/>  WHERE year = current_year<br/><br/>journal_no =<br/>  'JNL-' + year + '-' +<br/>  LPAD(sequence, 5, '0')<br/><br/>Example: 'JNL-2024-00001'"]
...
flowchart TD
    ...
    ValidationPass -->|Yes| ConfirmDialog["Show confirmation dialog:<br/><br/>'Post Adjustment ADJ-2024-001?<br/><br/>This action will:<br/>✓ Generate journal entries<br/>✓ Update stock balances<br/>✓ Lock the adjustment<br/>✓ Post to General Ledger<br/><br/>This cannot be undone.<br/>Continue?'<br/><br/>Buttons: [Cancel] [Post]"]
    ...
    CommitTransaction --> ShowSuccess["Show success message:<br/><br/>'Adjustment ADJ-2024-001<br/>posted successfully!'<br/><br/>Details:<br/>✓ Journal: JNL-2024-00001<br/>✓ Stock balances updated<br/>✓ GL posted<br/><br/>Button: [View Adjustment]"]
...
flowchart TD
    ...
    CommitTransaction --> ShowSuccess["Show success message:<br/><br/>'Adjustment ADJ-2024-001<br/>voided successfully!'<br/><br/>Details:<br/>✓ Journal reversed<br/>✓ Stock balances reversed<br/>✓ GL posting reversed<br/>✓ Void reason saved<br/><br/>Button: [View Adjustment]"]
